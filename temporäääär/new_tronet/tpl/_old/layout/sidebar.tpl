...
[{block name="sidebar"}]
    ...
    [{block name="sidebar_categoriestree"}]
      [{if $oView->getClassName() != 'search' && !preg_match('/^account/', $oView->getClassName()) }]
        [{if $oxcmp_categories }]
            [{include file="widget/sidebar/categoriestree.tpl" categories=$oxcmp_categories->getArray() act=$oxcmp_categories->getClickCat() deepLevel=0}]
        [{/if}]
      [{/if}]
    [{/block}]
    [{*if !preg_match('/^account/', $oView->getClassName()) *}]
      [{block name="sidebar_manufacturerssliderall"}]
          [{include file="widget/sidebar/manufacturerssliderall.tpl" }]
      [{/block}]
      
      [{block name="sidebar_katalogbox"}]
          [{include file="widget/sidebar/katalogbox.tpl" }]
      [{/block}]
      
      [{block name="sidebar_zahlungen"}]
          [{include file="widget/sidebar/zahlung.tpl" }]
      [{/block}]
