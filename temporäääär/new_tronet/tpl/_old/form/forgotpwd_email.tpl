...
<form class="js-oxValidate" action="[{ $oViewConf->getSelfActionLink() }]" name="order" method="post">
  ...
  <ul class="form clear">
    ...
    <li class="formSubmit">
        <button class="submitButton button" type="submit" title="[{ oxmultilang ident="PAGE_ACCOUNT_FORGOTPWD_REQUESTPWD"}]">[{ oxmultilang ident="PAGE_ACCOUNT_FORGOTPWD_REQUESTPWD"}]</button>
    </li>
  </ul>
</form>
