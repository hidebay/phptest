...
<div class="ordersteps[{if !$oViewConf->showFinalStep() }] nofinalstep[{/if}]">
    ...
    <dl class="[{ if $highlight == 4}]active[{/if}][{if !$oViewConf->showFinalStep() }] lastinrow[{/if}]">
        <dt>
            [{if $showStepLinks}]<a id="test_Step4" rel="nofollow" href="[{ if $oViewConf->getActiveClassName() == "payment"}]javascript:submitActiveForm();[{else}][{ oxgetseourl ident=$oViewConf->getOrderConfirmLink() }][{/if}]">[{/if}]
            <big>4.</big> [{ oxmultilang ident="INC_STEPS_ITEM_ORDER" }]
            [{if $showStepLinks}]</a>[{/if}]
        </dt>
        <dd>
            [{if $showStepLinks}]<a id="test_Step4_Text" rel="nofollow" href="[{ if $oViewConf->getActiveClassName() == "payment"}]javascript:submitActiveForm();[{else}][{ oxgetseourl ident=$oViewConf->getOrderConfirmLink() }][{/if}]">[{/if}][{ oxmultilang ident="INC_STEPS_ITEM_SUBMITORDER" }][{if $showStepLinks}]</a>[{/if}]
        </dd>
    </dl>
