                        ...
                        [{if $osubcat->getIsVisible() }]
                            ...
                            [{if $osubcat->getIsVisible() }]
                                <li[{if $osubcat->expanded}] class="current"[{/if}]>
                                    <a [{if $osubcat->expanded}]class="current"[{/if}] href="[{$osubcat->getLink()}]">[{$osubcat->oxcategories__oxtitle->value}] [{ if $oView->showCategoryArticlesCount() && ($osubcat->getNrOfArticles() > 0)}] ([{$osubcat->getNrOfArticles()}])[{/if}]</a>
                                      ...
                                      <div class="sub">
                                        ...
                                        [{foreach from=$osubcat->getSubCats() item=osubsubcat key=subsubcatkey name=SubSubCat}] 
                                          [{if $osubsubcat->getIsVisible() }]
                                            [{assign var="col_counter" value=$col_counter+1}]
                                            <li class="subsub">
                                              <a [{if $osubsubcat->expanded}]class="current"[{/if}] href="[{$osubsubcat->getLink()}]">[{$osubsubcat->oxcategories__oxtitle->value}] [{ if $oView->showCategoryArticlesCount() && ($osubsubcat->getNrOfArticles() > 0)}] ([{$osubsubcat->getNrOfArticles()}])[{/if}]</a>
                                            </li>
                                          [{/if}]                                          
                                        [{/foreach}]
                                        </ul>
