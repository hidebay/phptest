[{if $toparticlelist}]
  [{capture name="slides"}]
    [{assign var="aTopArticles" value=$toparticlelist}]
    [{assign var="iNumOfArticles" value=$aTopArticles|@count}]
    [{foreach from=$aTopArticles item=oArticle}]
...
[{/capture}]
    <h2 class="sectionHead clear">
        <span>[{ oxmultilang ident="TRO_MORE_PRODUCTS" }]</span>		
    </h2>
    [{oxscript include="js/libs/jcarousellite.js"}]
    [{oxscript include="js/widgets/trotoparticlesslider.js" priority=10 }]
    [{oxscript add="$( '#toparticlesSlider' ).troTopArticlesSlider();"}]
