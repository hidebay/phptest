<div>
    ...
    [{block name="widget_header_servicebox_flyoutbox"}]
        <p id="servicesTrigger" class="[{if $notificationsCounter > 0}]hasNotifications[{/if}]">
            <a href="[{ oxgetseourl ident=$oViewConf->getSslSelfLink()|cat:"cl=account" }]" rel="nofollow"><span>[{oxmultilang ident="WIDGET_SERVICES_ACCOUNT"}]</span></a>[{if $notificationsCounter > 0}]<span class="counter FXgradOrange">[{$notificationsCounter}]</span>[{/if}]
        </p>
        [{*<div class="flyoutBox">
