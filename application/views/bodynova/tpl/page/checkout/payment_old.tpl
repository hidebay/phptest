    ...
    [{block name="checkout_payment_main"}]
      ...
      <div class="checkoutCollumns clear">
        ...
        [{block name="change_shipping"}]
            [{ if $oView->getAllSets() }]
                <h3 id="deliveryHeader" class="blockHead">
                	[{ if $oView->getAllSetsCnt() > 1 }]
                		[{ oxmultilang ident="PAGE_CHECKOUT_PAYMENT_SELECTSHIPPING" }]
					[{else}]
                		[{ oxmultilang ident="PAGE_CHECKOUT_PAYMENT_SELECTEDSHIPPING" }]
					[{/if}]
                </h3>
