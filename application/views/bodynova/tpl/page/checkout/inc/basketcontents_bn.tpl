                    ...
                    [{block name="checkout_basketcontents_basketitem_wrapping"}]
                        ...
                        <td>
                                [{ if !$basketitem->getWrappingId() }]
                                    [{if $editable }]
                                        <a class="wrappingTrigger" rel="nofollow" href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=wrapping" params="aid="|cat:$basketitem->getProductId() }]" title="[{ oxmultilang ident="ADD" }]">[{ oxmultilang ident="ADD" }]</a>
                                    [{else}]
                                ...
                                [{else}]
                                    ...
                                    [{if $editable }]
                                        <a class="wrappingTrigger" rel="nofollow" href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=wrapping" params="aid="|cat:$basketitem->getProductId() }]" title="[{ oxmultilang ident="ADD" }]">[{$oWrap->oxwrapping__oxname->value}]</a>
                                    [{else}]
                ...
                <table>
                    [{if !$oxcmp_basket->getDiscounts() }]
                        ...
                        [{block name="checkout_basketcontents_nodiscountproductvats"}]
                            [{foreach from=$oxcmp_basket->getProductVats() item=VATitem key=key }]
                                <tr>
                                    <th>[{ oxmultilang ident="VAT_PLUS_PERCENT_AMOUNT" }]&nbsp;[{ $key }][{ oxmultilang ident="VAT_PLUS_PERCENT_AMOUNT" }]</th>
                    ...
                    [{block name="checkout_basketcontents_voucherdiscount"}]
                        [{if $oViewConf->getShowVouchers() && $oxcmp_basket->getVoucherDiscValue() }]
                            [{foreach from=$oxcmp_basket->getVouchers() item=sVoucher key=key name=Voucher}]
                                <tr class="couponData">
                                    <th><span><strong>[{ oxmultilang ident="COUPON" }]</strong>&nbsp;([{ oxmultilang ident="NUMBER2" }] [{ $sVoucher->sVoucherNr }])</span>
                                    [{if $editable }]
                    ...
                    [{block name="checkout_basketcontents_delcosts"}]
                        [{if $oxcmp_basket->getDelCostNet() }]
                            <tr>
                                <th>[{if $oxcmp_basket->getDelCostVat() }][{ oxmultilang ident="SHIPPING_NET" }][{else}][{ oxmultilang ident="SHIPPING_COST" }][{/if }]</th>
                                <td>[{ $oxcmp_basket->getDelCostNet() }]&nbsp;[{ $currency->sign }]</td>
                            </tr>
                            [{if $oxcmp_basket->getDelCostVat() }]
                                <tr>
                                    <th>[{ oxmultilang ident="BASKET_TOTAL_PLUS_PROPORTIONAL_VAT" }] [{ $oxcmp_basket->getDelCostVatPercent() }][{ oxmultilang ident="BASKET_TOTAL_PLUS_PROPORTIONAL_VAT" }]</th>
