...
[{capture append="oxidBlock_content"}]
        ...
        <div id="orderAddress">
            <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
              <h3 class="section">
                ...
                <button type="submit" class="change">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MODIFYADDRESS" }]</button>
                <div class="clear"></div>
              </h3>
            </form>
            <dl>
    ...
    [{block name="shippingAndPayment"}]
        ...
        <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
            <h3 class="section">
                ...
                <input type="hidden" name="fnc" value="">
                <button type="submit" class="change">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MODIFY2" }]</button>
                <div class="clear"></div>
            </h3>
        </form>
        ...
        <div id="orderPayment">
            <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
                <h3 class="section">
                    ...
                    <input type="hidden" name="cl" value="payment">
                    <input type="hidden" name="fnc" value="">
                    <button type="submit" class="change">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MODIFY3" }]</button>
                    <div class="clear"></div>
                </h3>
            </form>
    ...
    [{block name="checkout_order_main"}]
        ...
        [{/if}]
        <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
            <h3 class="section basket">
                <strong>[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_BASKET" }]</strong>
                [{ $oViewConf->getHiddenSid() }]
                <input type="hidden" name="cl" value="basket">
                <input type="hidden" name="fnc" value="">
                <button type="submit" class="change">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_MODIFY4" }]</button>
                <span>[{ oxmultilang ident="TRO_SUBMIT_HINT" }]</span>
                <div class="clear"></div>
            </h3>
        </form>
                ...
                [{else}]
                    [{block name="checkout_order_btn_confirm_bottom"}]
                        <form action="[{ $oViewConf->getSslSelfLink() }]" method="post" id="orderConfirmAgbBottom">
                            ...
                            <div class="lineBox clear">
                                <a href="[{ oxgetseourl ident=$oViewConf->getPaymentLink() }]" class="prevStep submitButton">[{ oxmultilang ident="PAGE_CHECKOUT_ORDER_BACKSTEP" }]</a>
                                <button type="submit" class="submitButton nextStep largeButton">[{ oxmultilang ident="TRO_PAGE_CHECKOUT_ORDER_SUBMITORDER" }]</button>
                            </div>
                        </form>
                    [{/block}]
                [{/if}]
...
[{include file="layout/page.tpl" title=$template_title location=$template_title sidebar="Left"}]