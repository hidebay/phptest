...
[{if $aVariantSelections && $aVariantSelections.rawselections}]
    ...
    [{foreach from=$aVariantSelections.rawselections item=oSelectionList key=iKey}]
        ...
        [{/if}]
    [{/foreach}]
    [{*}]<!--old [{oxscript add="var oxVariantSelections  = [`$_sSelectionHashCollection`];"}]-->[{*}]
...
<div class="detailsInfo clear">
    ...
    [{block name="details_productmain_zoom"}]
        [{if $oView->showZoomPics()}]
            ...
            <div class="picture">
                <a href="[{$oPictureProduct->getMasterZoomPictureUrl(1)}]" class="cloud-zoom" id="zoom1" rel="adjustY:-2, zoomWidth:'354', fixZoomWindow:'350', trImg:'[{$oViewConf->getImageUrl('dot.png')}]', loadingText:'[{oxmultilang ident="PAGE_DETAILS_ZOOM_LOADING"}]'">
...
[{*????*}]
        ...
        <div class="productMainInfo[{if $oManufacturer->oxmanufacturers__oxicon->value}] hasBrand[{/if}]">
            ...
            [{block name="details_productmain_artnumber"}]
                <span id="productArtnum" class="itemCode">[{oxmultilang ident="PRODUCT_NO"}] [{$oDetailsProduct->oxarticles__oxartnum->value}]</span>
            [{/block}]
            ...
            [{if $aVariantSelections && $aVariantSelections.selections }]
                ...
                [{if $blHasActiveSelections}]
                    <div class="variantReset">
                        [{*}]<!--[{ Reset link }]-->[{*}]
                        <a class="reset">[{ oxmultilang ident="RESET_SELECTION" }]</a>

                        [{*}]<!--[{ Active selections }]-->[{*}]
                        <label>[{ oxmultilang ident="SELECTED_COMBINATION" }]</label>
        ...
        <div class="tobasket" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
            [{if !$blCanBuy }]
                <p class="hint">
                  [{oxmultilang ident="TRO_CHOOSE_ATRCILE"}]
                </p>
            [{/if}]
            [{*}]<!--[{ pers params }]-->[{*}]
...
[{*???*}]                
                  ...
                  [{assign var=tprice value=$oDetailsProduct->getTPrice()}]
                    ...
                    [{if $tprice && $tprice->getBruttoPrice() > $price->getBruttoPrice()}]
                        <p class="oldPrice">
                          ...
                          [{$oDetailsProduct->getFTPrice()}] [{$currency->sign}]
                        </p>
                    [{/if}]
                [{/oxhasrights}]
            [{/block}]
                    ...
                    [{oxhasrights ident="TOBASKET"}]
                        [{if !$oDetailsProduct->isNotBuyable()}]
                          [{if $blCanBuy}]
                            ...
                            <button id="toBasket" type="submit" [{if !$blCanBuy}]disabled="disabled"[{/if}] class="submitButton button[{if !$blCanBuy}] inactive[{/if}]" title="[{oxmultilang ident="DETAILS_ADDTOCART"}]">[{oxmultilang ident="ADD_TO_CART"}]</button>
                          [{/if}]
                        [{/if}]
                    [{/oxhasrights}]
                [{/block}]
            ...
            [{if $oxcmp_user }]
                  <div class="merkzettel"><a id="linkToNoticeList" href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl="|cat:$oViewConf->getActiveClassName() params="aid=`$oDetailsProduct->oxarticles__oxnid->value`&amp;anid=`$oDetailsProduct->oxarticles__oxnid->value`&amp;fnc=tonoticelist&amp;am=1"|cat:$oViewConf->getNavUrlParams() }]" rel="nofollow">[{ oxmultilang ident="PAGE_DETAILS_ADDTONOTICELIST" }]</a></div>
            [{else}]
                  <div class="merkzettel"><a id="loginToNotice" href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account" params="anid=`$oDetailsProduct->oxarticles__oxnid->value`"|cat:"&amp;sourcecl="|cat:$oViewConf->getActiveClassName()|cat:$oViewConf->getNavUrlParams() }]" rel="nofollow">[{ oxmultilang ident="LOGIN_TO_ACCESS_WISH_LIST" }]</a></div>
            [{/if}]
            [{*}]<!--[{ additional info }]-->[{*}]
