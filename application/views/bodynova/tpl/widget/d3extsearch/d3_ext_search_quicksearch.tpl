...
<form action='[{$oViewConf->getSelfActionLink()}]' method='POST' name='basket'>
    ...
    *}][{* Achtung, nicht $shop->cl verwenden, statt dessen [{$sClassname}]  *}][{*
    ...
    <input type='image' src='[{$oViewConf->getImageUrl()}]/tobasket_button.gif' alt='[{oxmultilang ident="D3_EXTSEARCH_QUICK_TOBASKET"}]' style='float: left; margin: 0px; margin-right: 2px; border-style: solid; border-color: #CCC; border-width: 1px;'>
</form>
*}]