...
class oePayPalOrder_PayPal extends oxAdminDetails
{
    ...
    public function processAction()
    {
        try {
            ...
            $oActionFactory = oxNew( 'oePayPalOrderActionFactory' );
            $oAction = $oActionFactory->createAction( $sAction );

            $oAction->setRequest( $oRequest );
            $oAction->setOrder( $this->getEditObject()->getPayPalOrder() );
            $oAction->setAuthorizationId( $this->getEditObject()->oxorder__oxtransid->value );
